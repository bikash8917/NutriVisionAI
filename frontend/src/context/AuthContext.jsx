  ...
  useEffect(() => {
    const syncUser = () => {
      ...
      setUser((currentUser) => ({
        ...(currentUser || {}),
        ...currentProfile,
        name: currentProfile.fullName || currentProfile.email || currentProfile.username || 'User',
    ...
    const bootstrapAuth = async () => {
      try {
        ...
        if (currentUser) {
          setUser({
            ...currentUser,
            name: currentUser.profile?.fullName || currentUser.fullName || currentUser.email || currentUser.username || 'User',
            avatar: currentUser.profile?.profileImage || currentUser.avatar || '',
          });
          await Promise.all([
  ...
  const value = useMemo(
    () => ({
      ...
      login: async (payload) => {
        ...
        setUser({
          ...nextUser,
          name: nextUser?.profile?.fullName || nextUser?.fullName || nextUser?.email || nextUser?.username || 'User',
          avatar: nextUser?.profile?.profileImage || nextUser?.avatar || '',
        });
        return nextUser;
      },
